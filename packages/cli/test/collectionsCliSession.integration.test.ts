...
it("collections CLI: insert → list → get → update → delete cycle", { timeout: 180_000 }, async () => {
...
`;
    ...
    try {
      ...
      // Get by ID
      const fetched = await session.getQueryBus().execute(new GetCollectionRowQuery("messages_cli_test", inserted.id));
      ...
      // Confirm deletion
      const notFound = await session.getQueryBus().execute(new GetCollectionRowQuery("messages_cli_test", inserted.id));
