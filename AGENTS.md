...
- **`execute` must return what the node produces on each output port**, not a generic “input plus a wrapper”:
  ...
  - **Binary / file data:** emit **`BinaryAttachment`s on `item.binary`** via **`ctx.binary.attach`** + **`ctx.binary.withAttachment`** (or explicit item-shaped returns). **Do not** put bulk file bytes in **`item.json`** as base64 or huge strings—run state persists **`json`** inline in the DB and it **grows with that string**, while binaries use **storage-backed blobs** and only **references** on the item. See **`docs/content/concepts/execution.mdx`** and **`docs/content/workflows/custom-nodes.mdx`**.
