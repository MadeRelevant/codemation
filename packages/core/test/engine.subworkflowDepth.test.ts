...
test(
  "first subworkflow child rejects when maxSubworkflowDepth is 0",
  async () => {
    ...
    const scheduled = await kit.engine.runWorkflow(wfParent, "start", items([{ v: 1 }]), undefined, { maxSubworkflowDepth: 0 });
