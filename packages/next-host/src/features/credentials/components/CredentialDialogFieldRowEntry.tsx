...
export type CredentialDialogFieldRowEntryProps = Readonly<{
  entry: CredentialDialogOrderedField;
...
export function CredentialDialogFieldRowEntry(props: CredentialDialogFieldRowEntryProps): ReactNode {
  const {
    entry,
    publicFieldValues,
      ...
      <div key={`public-${field.key}`} className="flex flex-col gap-2">
        ...
        {lockedByEnv && envVarNameTrimmed(field) ? (
          <div data-testid={`credential-public-${field.key}`}>
            <CredentialEnvFieldStatusRow kind="managed" envVarName={envVarNameTrimmed(field)!} fieldKey={field.key} />
          </div>
        ) : null}
