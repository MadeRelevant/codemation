...
- **Plugin `defineNode(...)`** follows the same per-item model: implement **`executeOne(args, context)`** (mapped **`input`**, **`item`**, **`ctx`**, etc.). Use **`defineBatchNode(...)`** when a helper-defined node still needs legacy **`run(items, …)`** batch semantics. **Config** should hold **static** wiring (credentials, retry policy, presentation); **per-item** behavior and API parameters belong in **inputs** / wire JSON and **`mapInput`**, consistent with built-in nodes.
