...
test("SplitNode expands each item into one output item per returned element", async () => {
  ...
  const items: Item[] = [{ json: { batch: [1, 2] } }, { json: { batch: [3] } }];
  const out = await new SplitNode().execute(items, CoreNodesTestContextFactory.create(config));
  assert.deepEqual(
...
test("SplitNode yields an empty main batch when every split is empty", async () => {
  ...
  const out = await new SplitNode().execute([{ json: {} }], CoreNodesTestContextFactory.create(config));
...
test("FilterNode keeps only items matching the predicate", async () => {
  ...
  const items: Item[] = [{ json: { n: 1 } }, { json: { n: 2 } }, { json: { n: 4 } }];
  const out = await new FilterNode().execute(items, CoreNodesTestContextFactory.create(config));
  assert.deepEqual(
...
test("FilterNode returns an empty batch when nothing matches", async () => {
  ...
  const out = await new FilterNode().execute([{ json: 1 }, { json: 2 }], CoreNodesTestContextFactory.create(config));
  assert.deepEqual(out.main, []);
});
...
test("AggregateNode reduces the batch to a single item on main", async () => {
  ...
  const items: Item[] = [{ json: { v: 1 } }, { json: { v: 2 } }, { json: { v: 3 } }];
  const out = await new AggregateNode().execute(items, CoreNodesTestContextFactory.create(config));
...
test("AggregateNode returns an empty main batch when the input batch is empty", async () => {
  ...
  const out = await new AggregateNode().execute([], CoreNodesTestContextFactory.create(config));
  assert.deepEqual(out.main, []);
});
...
test("AggregateNode allows async aggregates", async () => {
  ...
  const out = await new AggregateNode().execute([{ json: 1 }], CoreNodesTestContextFactory.create(config));
