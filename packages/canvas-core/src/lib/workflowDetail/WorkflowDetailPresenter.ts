...
import { HumanFriendlyTimestampFormatter } from "../HumanFriendlyTimestampFormatter";
import type {
...
export type { RunWorkflowResult, RunWorkflowMode, RunWorkflowRequest };
...
export class WorkflowDetailPresenter {
  ...
  static async runWorkflow(
    ...
    return await apiClient.postRunWorkflow(workflowId, {
      items,
  ...
  static async runNode(
    ...
    return await apiClient.postRunNode(runId, nodeId, items, mode, synthesizeTriggerItems);
  }

  static async updatePinnedInput(
    apiClient: WorkflowCanvasApiClient,
    runId: string,
    nodeId: string,
    items: Items | undefined,
  ): Promise<PersistedRunState> {
    return await apiClient.patchRunNodePin(runId, nodeId, items);
  }
  ...
  static async updateWorkflowSnapshot(
    ...
    return await apiClient.patchRunWorkflowSnapshot(runId, workflowSnapshot);
  }
  ...
  static async uploadOverlayPinnedBinary(
    ...
    const body = await apiClient.postWorkflowDebuggerOverlayBinaryUpload(args.workflowId, {
      nodeId: args.nodeId,
  ...
  static async replaceWorkflowDebuggerOverlay(
    ...
    return await apiClient.putWorkflowDebuggerOverlay(workflowId, currentState);
  }
  ...
  static async copyRunToDebuggerOverlay(
    ...
    return await apiClient.postWorkflowDebuggerOverlayCopyRun(workflowId, sourceRunId);
  }
  ...
  static sortPortEntries(portMap: Readonly<Record<string, Items>> | undefined): PortEntries {
    if (!portMap) return [];
    const entries = Object.entries(portMap) as Array<[string, Items]>;
  ...
  static applyPinnedOutputsToPortEntries(
    ...
    const entries = Object.entries({ ...Object.fromEntries(portEntries), ...pinnedOutputsByPort }) as Array<
      [string, Items]
    >;
  ...
  static resolveSelectedPort(portEntries: PortEntries, current: string | null): string | null {
    if (!portEntries.length) {
      return null;
    }
    if (current && portEntries.some(([portName]) => portName === current)) {
      return current;
    }
    return portEntries[0]?.[0] ?? null;
  }
  ...
  static resolveInspectorNodeIdForCanvasPick(
    ...
    const matching = snapshots.filter((snapshot) => snapshot.nodeId === canvasWorkflowNodeId);
