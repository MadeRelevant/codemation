...
describe("CodemationPluginDiscovery", () => {
  ...
  afterEach(async () => {
    delete process.env.CODEMATION_PREFER_PLUGIN_SOURCE_ENTRY;
  ...
  it("prefers source plugin entry when development preference is enabled", async () => {
    process.env.CODEMATION_PREFER_PLUGIN_SOURCE_ENTRY = "true";
    const consumerRoot = await mkdtemp(path.join(os.tmpdir(), "codemation-plugin-discovery-src-"));
    tempRoots.push(consumerRoot);
    const packageRoot = path.join(consumerRoot, "node_modules", "@codemation", "example-plugin");
    await mkdir(path.join(packageRoot, "dist"), { recursive: true });
    ...
    await writeFile(
      ...
      [
        "const plugin = {",
        "  register() { throw new Error('dist entry should not load'); },",
        "};",
    ...
    await writeFile(
      path.join(packageRoot, "codemation.plugin.ts"),
      ["const plugin = {", "  register() {},", "};", "export default plugin;", ""].join("\n"),
      "utf8",
    );
