...
import { CodemationDialog, JsonMonacoEditor } from "@codemation/ui";
import { Button } from "@codemation/ui";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@codemation/ui";
...
export function WorkflowJsonEditorDialog(
  ...
  const handleUploadOrReplace = useCallback(
    async (itemIndex: number, file: File, attachmentName: string) => {
      ...
      try {
        const attachment = await WorkflowDetailPresenter.uploadOverlayPinnedBinary(apiClient, {
