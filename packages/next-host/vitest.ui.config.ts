...
/**
 * UI suite config for @codemation/next-host.
 * Scoped to *.test.tsx (jsdom) — companion to vitest.config.ts (mixed-env unit suite).
 */
export default defineConfig({
  ...
  test: {
    ...
    coverage: {
      provider: "v8",
      // Force all source files into the denominator so per-suite % matches merged lcov.
      all: true,
