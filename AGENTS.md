...
- Activations are still **batch-shaped** (`Items` on `main`), but implementations differ:
  ...
  - **Per-item nodes** (`ItemNode`, `executeOne`)—for example built-in **`MapDataNode`** and **`AIAgentNode`**—let the engine iterate items and apply optional **`mapInput` / `inputSchema`** before enqueue so run snapshots show **effective inputs**. See **`packages/core/docs/item-node-execution.md`**.
