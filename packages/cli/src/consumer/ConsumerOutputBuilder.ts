...
export class ConsumerOutputBuilder {
  ...
  private async writeEntryFile(
    ...
    const outputText = [
      ...
      "      const workflows = this.resolveWorkflowModuleExports(workflowModule);",
      ...
      "      const lines = workflowSourcePaths.map((p) => `  - ${p}`).join('\\n');",
      "      throw new Error(",
      '        ["Discovered " + workflowSourcePaths.length + " file(s) under workflow discovery, but none export a WorkflowDefinition.", lines, "", "Move shared helpers outside the discovery directories (for example src/lib), or export at least one object with id, name, nodes, and edges from a workflow module."].join("\\n")',
      "      );",
      "    }",
      "    return [...workflowsById.values()];",
      "  },",
      "  resolveWorkflowModuleExports(moduleExports) {",
      "    return Object.values(moduleExports).filter((value) => this.isWorkflowDefinition(value));",
      "  },",
