/**
 * Bundle-boundary enforcement for @codemation/canvas.
 *
 * Walks the import graph of packages/canvas/src/** and asserts that no file
 * imports from server-only modules that must not appear in a browser bundle.
 *
...
describe("@codemation/canvas bundle boundary", () => {
  const files = collectSourceFiles(SRC_ROOT);
  expect(files.length).toBeGreaterThan(0);

  it("no source file imports server-only modules", () => {
    ...
    if (violations.length > 0) {
      ...
      expect.fail(`Server-only imports found in @codemation/canvas/src:\n${message}`);
    }
  });
});
