...
import { HUMAN_APPROVAL_MARKER } from "./humanApprovalMarker.types";
...
export class AIAgentNode implements RunnableNode<AIAgent<any, any>> {
  ...
  private resolveHumanApprovalBehavior(config: ToolConfig): Readonly<{ onRejected: "halt" | "return" }> | undefined {
    ...
    const nodeConfig = config.node as unknown as Record<symbol, unknown>;
    const marker = nodeConfig[HUMAN_APPROVAL_MARKER];
    if (marker === undefined) return undefined;
    const behavior = marker as { onRejected?: "halt" | "return" };
    return { onRejected: behavior.onRejected ?? "return" };
  }
   ...
   * HITL tools (detected via {@link HUMAN_APPROVAL_MARKER}) get the solo-constraint sentence
