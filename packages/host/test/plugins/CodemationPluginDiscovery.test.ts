...
describe("CodemationPluginDiscovery", () => {
  ...
  it("discovers the codemation.plugin manifest and resolves the default export", async () => {
      ...
      JSON.stringify(
        {
          ...
          codemation: {
            plugin: {
              kind: "plugin",
              entry: "./dist/codemation.plugin.js",
            },
          },
        },
        null,
    ...
    expect(resolvedPackages[0]?.packageName).toBe("@codemation/example-plugin");
  });
});
