...
1. **`await ctx.binary.attach({ name, body, mimeType, filename? })`** — uploads bytes. **`body`** may be `Uint8Array`, `ArrayBuffer`, a **`ReadableStream`**, or an async iterable of `Uint8Array` chunks (same idea as **`HttpRequestNode`** streaming a download).
2. **`ctx.binary.withAttachment(baseItem, name, attachment)`** — returns an item whose **`binary[name]`** points at storage.
