...
Do not rely on consumers executing `codemation.plugin.ts` from your published package. Prefer publishing only the built output and package metadata, for example:
...
If you do publish `codemation.plugin.ts` for reference, treat it as source-only. The supported runtime entry for discovery remains `package.json#codemation.plugin` pointing at built JavaScript.
