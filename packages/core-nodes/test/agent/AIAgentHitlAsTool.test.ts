...
import { HUMAN_APPROVAL_MARKER } from "../../src/nodes/humanApprovalMarker.types";
...
/** A node config whose `node` carries the HUMAN_APPROVAL_MARKER. */
class HitlNodeConfig implements RunnableNodeConfig<Record<string, unknown>, Record<string, unknown>> {
  ...
  readonly [HUMAN_APPROVAL_MARKER] = { onRejected: "return" as const };
...
class HitlNodeConfigHalt implements RunnableNodeConfig<Record<string, unknown>, Record<string, unknown>> {
  readonly kind = "node" as const;
  readonly type: TypeToken<unknown> = HitlNode;
  readonly [HUMAN_APPROVAL_MARKER] = { onRejected: "halt" as const };
