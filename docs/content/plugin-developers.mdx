...
Treat `codemation.plugin.ts` as a repository authoring entry, not as the consumer runtime entry. Prefer publishing `dist/**` and package metadata only, or at minimum make sure the published `codemation.plugin` manifest field still points at the built JavaScript plugin module.
