-- Test-suite persistence (Postgres).
...
-- Run.workflowSnapshotJson already snapshots the workflow definition at run time, so historical
-- viewing of a renamed/deleted TestTrigger node still resolves through the existing Run row.

ALTER TABLE "Run"
  ADD COLUMN "test_suite_run_id" TEXT,
...
ALTER TABLE "Run"
  ADD CONSTRAINT "Run_test_suite_run_id_fkey"
  FOREIGN KEY ("test_suite_run_id")
  REFERENCES "TestSuiteRun"("id")
...
CREATE TABLE "TestAssertion" (
  ...
  CONSTRAINT "TestAssertion_run_id_fkey"
    FOREIGN KEY ("run_id") REFERENCES "Run"("run_id")
    ON DELETE CASCADE ON UPDATE CASCADE,
  CONSTRAINT "TestAssertion_test_suite_run_id_fkey"
    FOREIGN KEY ("test_suite_run_id") REFERENCES "TestSuiteRun"("id")
    ON DELETE CASCADE ON UPDATE CASCADE
);
