...
- **`defineNode({ … })`** generates an **`ItemNode`**: implement **`executeOne(args, context)`** once per item. **`args`** includes **`input`** (after **`inputSchema`** parse), **`item`**, **`itemIndex`**, **`items`**, and **`ctx`**. **`context`** exposes **`config`** (with **`itemValue`** leaves resolved), **`credentials`**, and **`execution`**. Optional **`inputSchema`** on the definition matches the engine’s Zod step; optional **`itemValue`** on config fields matches the engine’s per-item resolution; **`TWireJson`** (third generic on **`RunnableNodeConfig`**) types upstream **`item.json`** for DSL wiring.
...
- **Config vs inputs**: keep **credentials, retry policy, static options** on **config**; **per-item** query/API behavior belongs in **inputs** / wire JSON (and optional **`itemValue`** on config fields), not duplicated as “config that changes every item.”
