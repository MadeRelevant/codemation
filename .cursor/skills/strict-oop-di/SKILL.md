...
- **Files / blobs:** add bytes with **`ctx.binary.attach`** + **`ctx.binary.withAttachment`** (or explicit **`item.binary`**). **Do not** embed bulk **base64** (or huge strings) in **`item.json`**—persisted run JSON in the DB grows with that data; binaries use **storage + references** only. See **`docs/content/concepts/execution.mdx`** and **`docs/content/workflows/custom-nodes.mdx`**.
