...
| Pinned-output skipping | Pinned outputs survive clear-from-node, preserve downstream satisfaction, and produce skipped snapshots instead of re-executing the pinned node. | `packages/core/test/engine.targetedExecution.test.ts`, `packages/host/test/workflowDetail/mutableExecutionFlows.test.tsx` |
