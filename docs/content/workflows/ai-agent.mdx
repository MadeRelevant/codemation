...
- use `itemValue(...)` for per-item message generation
- use the object form when you need `prompt` plus `buildMessages`

If you want to reshape workflow data before the agent runs, use fluent DSL helpers such as `.map((item, ctx) => ...)`: read the current row from `item.json`, and use `ctx.data` when prompt preparation depends on earlier completed steps. Keep `itemValue(...)` for agent config fields like `messages`.
...
`workflow().agent(...)` is the fluent workflow authoring form of the agent step. `new AIAgent(...)` exposes the same message model as a direct node config.
...
For workflow-defined agents, prefer `itemValue(...)` when messages depend on the current item.
