"use client";

import { Component, type ReactNode } from "react";
import type { SimpleIcon } from "simple-icons";
import { siDotnet, siGithub, siOpenid } from "simple-icons";

/**
 * Standard color Google "G" (multicolor). Simple Icons only ships a monochrome mark;
 * Google requires the full-color logo for sign-in UI.
 *
 * @see https://developers.google.com/identity/branding-guidelines
 */
function GoogleColorGIcon(props: Readonly<{ className?: string; testId?: string }>): ReactNode {
  return (
    <svg
      role="img"
      className={props.className}
      viewBox="0 0 24 24"
      xmlns="http://www.w3.org/2000/svg"
      aria-hidden
      data-testid={props.testId}
    >
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92c-.26 1.37-1.04 2.53-2.21 3.31v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.09z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  );
}

/**
 * Maps Auth.js / NextAuth provider `id` to Simple Icons data.
 * Unknown ids fall back to OpenID for custom OIDC providers.
 *
 * Note: Microsoft marks are not in Simple Icons; Entra / legacy Azure AD use `.NET` as the closest ecosystem mark.
 */
const NEXT_AUTH_PROVIDER_ICONS: Readonly<Record<string, SimpleIcon>> = {
  github: siGithub,
  "microsoft-entra-id": siDotnet,
  "azure-ad": siDotnet,
};

function simpleIconForProvider(providerId: string): SimpleIcon {
  return NEXT_AUTH_PROVIDER_ICONS[providerId] ?? siOpenid;
}

export type OauthProviderIconProps = Readonly<{
  providerId: string;
  className?: string;
  testId?: string;
}>;

export class OauthProviderIcon extends Component<OauthProviderIconProps> {
  override render(): ReactNode {
    if (this.props.providerId === "google") {
      return (
        <GoogleColorGIcon className={this.props.className} testId={this.props.testId} />
      );
    }
    const icon = simpleIconForProvider(this.props.providerId);
    return (
      <svg
        role="img"
        className={this.props.className}
        viewBox="0 0 24 24"
        xmlns="http://www.w3.org/2000/svg"
        aria-hidden
        data-testid={this.props.testId}
      >
        <path d={icon.path} fill={`#${icon.hex}`} />
      </svg>
    );
  }
}
